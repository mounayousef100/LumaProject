		...
		for (int i = 0; i < Search.size(); i++) {
			...
			boolean checkSearch = result.contains("Jackets");
				mysoft.assertEquals(checkSearch, true,
						"check All the items displayed contains Jackets .");
				mysoft.assertAll();
		}
	}
	...
	public void CheckPrice() throws Throwable {
		...
		for (int i = 0; i < SearchPrice.size(); i++) {
			...
			if (FinalResult >= minPrice && FinalResult <= maxPrice) {
				...
				mysoft5.assertEquals(checkSearch, true, "check All the items displayed price between $70.00 - $79.99 .");
				mysoft5.assertAll();
			}
